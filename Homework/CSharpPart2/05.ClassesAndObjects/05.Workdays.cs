...
class Workdays
{
    ...
    static void GetWorkdaysBetween(DateTime start, DateTime end)
    {
        ...
        DateTime[] holidays = {
        new DateTime(start.Year - 1, 12, 31),
        new DateTime(start.Year - 1,  1,  1),
        new DateTime(start.Year - 1,  3,  3),
        new DateTime(start.Year - 1,  5,  6),
        new DateTime(start.Year - 1,  5, 24),
        new DateTime(start.Year - 1, 10,  9), }; // My birthday, obv. 
                                                                                // If it is Sat/Sun, we
        ...
        {
            Console.WriteLine(y + start.Year);
            ...
            for (int i = 0; i < holidays.Length; i++)
            {
                ...
                if (holidays[i] <= end && holidays[i] >= start &&
                    holidays[i].DayOfWeek != DayOfWeek.Saturday && holidays[i].DayOfWeek != DayOfWeek.Sunday)
                        Console.WriteLine("{0:MMM dd, yyyy} has been excluded as a holiday.", holidays[i], resultDays--);
            }
